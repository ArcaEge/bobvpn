...
echo "--- Starting client with --force-http ---"
docker run -d --name bobvpn-client \
    ...
    -e RUST_LOG=info \
    bobvpn-test \
    client --server bobvpn-server --insecure --force-http
...
if docker logs bobvpn-client 2>&1 | grep -qi "error.*reconnecting\|error.*panic\|error.*failed"; then
    echo "ERROR: Client reported errors"
    exit 1
fi

if docker logs bobvpn-client 2>&1 | grep -q "tunnel established"; then
    echo "SUCCESS: Handshake completed via HTTP fallback!"
else
    echo "FAIL: No handshake completion detected"
    docker logs bobvpn-client 2>&1
    exit 1
fi
...
if docker ps --format '{{.Names}}' | grep -q bobvpn-client &&
...
echo ""
echo "=== ALL HTTP FALLBACK TESTS PASSED ==="
